...
export function CommunityPanel({ onOpenProject }: CommunityPanelProps) {
  ...
  const [currentUserId, setCurrentUserId] = useState<string>('jason');

  useEffect(() => {
    backend.auth.getUser().then((user) => {
      const uid = user?.id || 'jason';
      setCurrentUserId(uid);
      setProfile(profileStore.getProfile(uid));
    }).catch(() => {
      setProfile(profileStore.getProfile('jason'));
    });
  }, []);
  ...
  const refresh = useCallback(async () => {
    setProfile(profileStore.getProfile(currentUserId));
  ...
  const saveProfile = () => {
    setError(null);
    try {
      const updated = profileStore.updateProfile(currentUserId, {
  ...
  const remix = async (entryId: string) => {
    setError(null);
    try {
      const record = await remixService.remixTemplate(entryId, currentUserId, projectStore);
      setNotice(
  ...
  const addComment = (targetId: string, parentId?: string) => {
    ...
    if (!text) return;
    try {
      commentService.add(targetId, currentUserId, text, parentId);
  ...
  const myTemplates = creatorTemplateStore.list().filter((r) => r.authorId === currentUserId);
