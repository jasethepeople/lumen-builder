...
export const backend: Backend = createBackend({ VITE_SUPABASE_URL: supabaseUrl, VITE_SUPABASE_ANON_KEY: supabaseAnonKey });
...
export async function backendUserLabel(): Promise<string | undefined> {
  try {
    ...
    return user?.email ?? user?.id ?? 'hosted@supabase';
  } catch {
